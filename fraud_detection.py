import streamlit as st
import pandas as pd
import joblib

# Load the model
model = joblib.load("fraud_detection_pipeline.pkl")

# Streamlit app title
st.title("Fraud Detection Prediction App")
st.markdown("Please enter the transaction details and use the predict button")
st.divider()

# Input fields with unique keys
transaction_type = st.selectbox("Transaction Type", ["PAYMENT", "TRANSFER", "CASH_OUT", "DEPOSIT"])
amount = st.number_input("Amount", min_value=0.0, value=1000.0, key="amount_input")
oldbalanceOrg = st.number_input("Old Balance(Sender)", min_value=0.0, value=10000.0, key="oldbalanceOrg_input")
newbalanceOrig = st.number_input("New Balance(Sender)", min_value=0.0, value=9000.0, key="newbalanceOrig_input")
oldbalanceDest = st.number_input("Old Balance(Receiver)", min_value=0.0, value=0.0, key="oldbalanceDest_input")
newbalanceDest = st.number_input("New Balance(Receiver)", min_value=0.0, value=0.0, key="newbalanceDest_input")

# Prediction button
if st.button("Predict"):
    input_data = pd.DataFrame([{
        "type": transaction_type,
        "amount": amount,
        "oldbalanceOrg": oldbalanceOrg,
        "newbalanceOrig": newbalanceOrig,
        "oldbalanceDest": oldbalanceDest,
        "newbalanceDest": newbalanceDest
    }])
    
    prediction = model.predict(input_data)[0]
    
    st.subheader(f"Prediction: '{int(prediction)}'")
    if prediction == 1:
        st.error("Fraud Alert")
    else:
        st.success("This transaction is not a fraud")